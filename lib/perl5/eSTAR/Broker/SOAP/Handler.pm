# SOAP layer ontop of eSTAR::UA::Handler
package eSTAR::Broker::SOAP::Handler;

use strict;
use vars qw( @ISA %COOKIES );

use SOAP::Lite;
use eSTAR::Broker::Handler;

@ISA = qw(eSTAR::Broker::Handler);
...
BEGIN {
  ...
  for my $method qw( ping echo handle_voevent get_option
                     set_option kill ) {
     eval "sub $method";
     *$method = sub {
        my $self = shift->new();
