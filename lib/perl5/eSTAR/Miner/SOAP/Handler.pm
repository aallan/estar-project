# SOAP layer ontop of eSTAR::Miner::Handler
package eSTAR::Miner::SOAP::Handler;

use strict;
use vars qw( @ISA %COOKIES );

use SOAP::Lite;
use eSTAR::Miner::Handler;
use eSTAR::Logging;

@ISA = qw(eSTAR::Miner::Handler);
...
BEGIN {
  ...
  for my $method qw( ping echo get_option set_option handle_objects ) {
     eval "sub $method";
     *$method = sub {
        my $self = shift->new();
