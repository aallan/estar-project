...
BEGIN {
  ...
  foreach (@ARGV) {
    if (/^-vers/) {
      print "\neSTAR/NOAO ESSENCE Client Software:\n";
...
my $process = new eSTAR::Process( "noao_event_client" );  
...
# Start of log file
$log->header("Starting eSTAR/NOAO Event Client: Version $VERSION");
...
my $incoming_callback = sub {
   ...
   unless( $id =~ "voevent.noao.edu" && $id =~ "essence" ) {
...
#   print Dumper( $coords );
   ...
   if( $event->role() eq "test" ) {
      $log->print("Recieved an ESSENCE 'test' message...");
      $log->debug( $message );
      $log->thread( "Client", "Done." );
      return ESTAR__FAULT;       
      ...
      # Submit observations
      # -------------------
      $log->print("Recieved an ESSENCE 'observation' message...");
      ...
      $log->print("Recieved an unknown ESSENCE message...");
      $log->debug( $message );
   }
