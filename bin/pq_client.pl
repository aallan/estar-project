...
BEGIN {
  ...
  foreach (@ARGV) {
    if (/^-vers/) {
      print "\neSTAR/Palomar-Quest Client Software:\n";
...
my $process = new eSTAR::Process( "pq_event_client" );  
...
# Start of log file
$log->header("Starting eSTAR/PQ Event Client: Version $VERSION");
...
if ( $config->get_state("ec.unique_process") == 1 ) {
   ...
   $config->set_option("ec.project", "Test" );
     
   # user agent
   $config->set_option("ua.host", 'estar4.astro.ex.ac.uk' );
...
unless( defined $opt{"project"} ) {
   ...
   if ( defined $config->get_option("ec.project") ) {
      $log->warn("Warning: Resetting project to $opt{project}");
...
my $incoming_callback = sub {
   ...
   unless( $id =~ "nvo.caltech.edu" && $id =~ "pqspecial" ) {
...
#   print Dumper( $coords );
   ...
   if( $event->role() eq "test" ) {
      $log->print("Recieved an PQ 'test' message...");
      $log->debug( $message );
      $log->thread( "Client", "Done." );
      return ESTAR__FAULT;       
      ...
      # Submit observations
      # -------------------
      $log->print("Recieved an PQ 'observation' message...");
      ...
      foreach my $i ( 0 ... $#filter ) {
         ...
         $log->print( "Making observation request for filter $filter[$i]...");
         ...
         if ( $@ ) {
            ...
            $log->error( "Error: $error" );
            $log->error( "Warning: Returning ESTAR__FAULT" );
            $log->thread( "Client", "Failed moving to next filter..." );
            next;
         }
      ...
      $log->print("Recieved an unknown PQ message...");
      $log->debug( $message );
   }
